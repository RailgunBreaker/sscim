...
>
> **This is the frozen specification of a superseded model.** It is preserved
> because `docs/benchmarks/v7-exposure-robustness-frozen-benchmark.json` holds
> the v7.0 output surface, and a frozen set of numbers with no readable
> definition behind it is not a record of anything.
>
> | | |
> | --- | --- |
> | Model identifier | `sscim-model-v7-exposure-robustness` |
> | Application version | `0.0.0` (no release version was set at v7.0) |
> | Dataset | `2026-09-04` |
...
> | Superseded by | **v7.1**, `sscim-model-v7.1-exposure-robustness` - [`docs/MODEL_V7_SPEC.md`](../../MODEL_V7_SPEC.md) |
>
> **Do not read this file as a description of what the code now does.** Four
> of its statements are known to be wrong, and each was corrected in v7.1;
> they are listed in [`README.md`](README.md) with the numerical consequence
> of each. The two that change published numbers are the network-influence
> definition and the benchmark ablation parameters.
>
> **What was changed when this file was archived, and nothing else:**
>
> 1. This banner was prepended.
> 2. Three relative links were repointed for the new folder depth, and one
>    of them now names the archived v7.0 validation artifact rather than the
>    regenerated v7.1 one. The link targets are listed in `README.md`.
>
> The body below is otherwise **byte-for-byte** the file at that commit.
> Its `<!-- BEGIN GENERATED -->` blocks are **frozen v7.0 renderings**: the
> documentation generator skips `docs/archive/`, so they are never
> refreshed against the current registry. `npm run docs:verify` enforces
> that skip.
...
| **Synthetic parameter recovery** | **Complete** | Three coefficients are recoverable under a synthetic, **correctly specified** data-generating process, and the optimization is implemented correctly. The DGP is the model. See [SYNTHETIC_PARAMETER_RECOVERY.md](validation/SYNTHETIC_PARAMETER_RECOVERY.md). |
