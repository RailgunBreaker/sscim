> # WARNING - HISTORICAL DOCUMENT - SSCIM MODEL v7.0 VALIDATION
>
> **These are the v7.0 synthetic-recovery artefacts, preserved unchanged.**
> They were produced by the v7.0 engine against the `2026-08-29` dataset and
> they are NOT a description of the current model.
>
> | | |
> | --- | --- |
...
> | Artefacts | `validation-results.json`, `mle_replications.csv`, `mc_robustness_draws.csv` in this folder |
> | Superseded by | [`docs/computation-demo/validation/SYNTHETIC_PARAMETER_RECOVERY.md`](../../../computation-demo/validation/SYNTHETIC_PARAMETER_RECOVERY.md), recomputed on v7.1 against `2026-09-04` |
>
> **Why this folder exists.** For a period the current validation page was
> relabelled v7.1 while the JSON beside it still said
> `sscim-model-v7-exposure-robustness` and `2026-08-29`. The prose and the
> numbers disagreed. The fix was to preserve the v7.0 numbers HERE with
> their real labels and to recompute the current set, rather than to relabel
> old output. Old numerical output is never renamed to a newer model.
>
> Only this banner and the relative-link depth were changed. The body is
> otherwise byte-for-byte the file at that commit.

# Synthetic parameter recovery

*Model version: `sscim-model-v7-exposure-robustness`. Canonical specification:
[`docs/MODEL_V7_SPEC.md`](../MODEL_V7.0_SPEC.md) — see
[§9, validation status](../MODEL_V7.0_SPEC.md#9-validation-status).*
...
The roadmap is [spec §11](../MODEL_V7.0_SPEC.md#11-limitations-and-calibration-roadmap).
