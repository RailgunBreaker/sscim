> **HISTORICAL DOCUMENT** - Original pre-review experiment, preserved from commit 207ecde. Current calculation report: [current validation](../../../computation-demo/validation/SYNTHETIC_PARAMETER_RECOVERY.md).
...
together and must agree. `npm run docs:verify` fails if they do not.

| | |
| --- | --- |
| Model version | `sscim-model-v7.1-exposure-robustness` |
| Dataset as-of | `2026-09-04` |
| Base chain index | 6.027797 |
| Assumption envelope (Part B) | [5.594663, 6.684435] over 200 draws |
| Structural Spearman vs base | mean 0.9971, min 0.9930 |
| Company-criticality Spearman vs base | mean 0.9896, min 0.9569 |
| Modal top-five companies held in | 71.5% of draws |
| Stage holding the structural top spot | `adv_fab` 100.0% |

**The v7.0 artefacts are not here.** They were computed by the superseded
engine against the `2026-08-29` dataset and are preserved with their own
labels in
[`docs/archive/v7.0/validation/`](../../v7.0/validation/SYNTHETIC_PARAMETER_RECOVERY.md).
Old numerical output is never relabelled to a newer model version; it is
either recomputed or archived.

**What v7.1 changed here.** Part A is unaffected: the synthetic recovery is
seeded and generated by the model itself, so it does not depend on the
dataset or on the network-influence definition. Part B moved, for two
reasons together — the dataset advanced from `2026-08-29` to `2026-09-04`,
and the v7.1 SpilloverReach correction changed the structural ordering. The
visible consequence is that the structural top spot is now held by
`adv_fab` in **100%** of draws, against a v7.0 split of 77.5% `adv_fab` /
22.5% `litho`. Removing a stage's own weight from its network-influence
score removed the tie that produced that split.
