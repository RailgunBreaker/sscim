...
The graph, snapshot, assumptions, and priors are versioned. Record the commit, snapshot date, event/scenario, and parameters when comparing results. SSCIM lacks facility-level capacity, inventory, bill-of-material, qualification, and recovery-time data; it is not calibrated to observed outcomes. See [Calculation specification](calculation/README.md), [Validation note](validation/MLE_VALIDATION.md), and [Model roadmap](MODEL_ROADMAP.md).
