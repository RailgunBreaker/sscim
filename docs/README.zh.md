...
事件会在选定环节注入冲击。引擎以 12 天半衰期对其衰减，使用已声明的下游与上游传导先验沿所有可达路径传播，用有界规则合成重叠的贡献，再进行汇总。
...
- **一次事件**与**关于它的多篇报道**。指数通过有界的 noisy-OR 累积事件：这对相互独立的事件是正确的，对同一事件的重复报道则是错误的。描述同一事件的多条记录会被归组——其中恰有一条计入分数，其余作为后续更新或恢复报告发布，各自保留自己的来源与评估。
