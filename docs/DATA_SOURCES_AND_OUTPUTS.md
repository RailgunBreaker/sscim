...
| Site-to-site network | ~890 modeled links | — | **Derived, not input.** Composed from the company `customers` edges and the stage flow graph — see below. No dataset here records which plant ships to which plant. |
...
- **Siblings are a third case.** Unimicron and Ibiden make the ABF substrates NVIDIA packages are built on, but substrates and logic_ai both feed advanced packaging and neither reaches the other. A forward-or-backward rule scored one of the most watched constraints in the industry at zero and dropped 12 real company relationships with it. Those links are now built as `co-input`, weighted by the *weaker* of the two paths to the meeting stage, and drawn dotted.
