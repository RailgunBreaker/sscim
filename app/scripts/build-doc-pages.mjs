...
import { THEMES, THEME_KEY, preferredTheme, applyTheme } from '../src/theme.js';
...
header{position:sticky;top:0;z-index:5;border-bottom:1px solid var(--line);background:var(--panel)}
...
.tree-title{display:flex;align-items:center;justify-content:space-between;margin:0 4px 8px;color:var(--faint);font-size:12px;font-weight:600;letter-spacing:1.1px}.tree-title a{color:var(--copper)}
...
.tree-folder{display:block;margin:10px 0 3px;color:var(--dim);font-size:12px;font-weight:600;letter-spacing:.5px}.tree-file{display:block;padding:5px 7px;border-radius:4px;color:var(--dim);font-size:12.5px;line-height:1.4;overflow-wrap:anywhere}.tree-file:hover{background:var(--panel);color:var(--text);text-decoration:none}.tree-file.current{background:var(--selected);color:var(--text);box-shadow:inset 2px 0 var(--copper)}
.sidebar-section+.sidebar-section{border-top:1px solid var(--line);margin-top:14px;padding-top:12px}.toc-list{list-style:none;margin:0;padding:0}.toc-list li{margin:3px 0}.toc-list a{display:block;padding:3px 7px;color:var(--faint);font-size:13px;line-height:1.45;overflow-wrap:anywhere}.toc-list a:hover{color:var(--text);background:var(--panel);border-radius:4px;text-decoration:none}.toc-list .level-3{padding-left:16px}.toc-list .level-4{padding-left:25px;color:var(--faint)}
...
.note{border:1px solid var(--line);background:var(--panel2);border-radius:7px;padding:12px 14px;margin:0 0 28px;color:var(--dim);font-size:13px}
.note b{color:var(--text)}
.markdown{color:var(--dim);font-size:15px;overflow-wrap:anywhere}
...
.markdown code{font:12px "Cascadia Mono","SFMono-Regular",Consolas,monospace;background:var(--panel2);border:1px solid var(--line);padding:1px 4px;border-radius:3px;color:var(--text)}
.markdown pre{background:var(--panel2);border:1px solid var(--line);border-radius:7px;padding:16px;overflow:auto;margin:18px 0}
.markdown pre code{border:0;background:transparent;padding:0;color:var(--text)}
.markdown blockquote{margin:18px 0;border-left:3px solid var(--amber);background:var(--panel2);padding:10px 14px}
...
// Vite bundles the same React controls and CSS used by the library.
const manifest = JSON.parse(await readFile(path.join(outDir, '.vite/manifest.json'), 'utf8'));
const pageEntry = manifest['src/docs/page.jsx'];
if (!pageEntry) throw new Error('Document UI bundle missing; run npm run build first.');
const pageStyles = new Set();
const seenAssets = new Set();
function collectStyles(entry) {
  if (seenAssets.has(entry)) return;
  seenAssets.add(entry);
  for (const file of entry.css || []) pageStyles.add(file);
  for (const imported of entry.imports || []) collectStyles(manifest[imported]);
}
collectStyles(pageEntry);
const modifiedDate = (doc) => `<span class="doc-modified"><span data-doc-label="Last modified">Last modified</span>: <time datetime="${escape(doc.modifiedAt)}">${escape(doc.modifiedAt.slice(0, 10))}</time></span>`;

const shell = ({ title, path: docPath, body, root, modifiedAt }) => `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1" />
${modifiedAt ? `<meta property="article:modified_time" content="${escape(modifiedAt)}" />` : ""}
...
<style>${STYLE}</style>
<script>{const THEMES=${JSON.stringify(THEMES)};const THEME_KEY=${JSON.stringify(THEME_KEY)};(${applyTheme.toString()})((${preferredTheme.toString()})());}</script>
${[...pageStyles].map((file) => `<link rel="stylesheet" href="${root}${file}" />`).join('\n')}
<script type="module" src="${root}${pageEntry.file}"></script>
</head>
<body>
<div id="doc-header" data-root="${escape(root)}"><header class="docs-header"><div class="docs-bar">
  <a class="docs-brand" href="${root}index.html" aria-label="SSCIM home"><img src="${root}sscim-logo.png" alt="SSCIM" /></a>
  <span class="docs-section">Documentation</span>
  <nav class="nav">
...
<div class="cards">${list.map((d) => `<a class="card" href="../${pageFor(d.path)}"><strong>${escape(d.title)}</strong><small>${escape(d.path)}</small>${modifiedDate(d)}<span class="tags">${d.tags.map((t) => `<span class="tag">${escape(labelFor(t))}</span>`).join('')}</span></a>`).join('')}</div>`).join('\n')}`;
