...
export const T = {
  ...
  layer1: {
    en: "<strong>Layer 1 — World Map.</strong> Real OpenStreetMap geography with all 16 countries participating in the chain. Node color and size show each country's structural vulnerability — a time-invariant score derived purely from its stage participation, never hand-set; hovering shows its separate operational impact and any applied hazard delta.",
    zh: '<strong>第一层 — 世界地图。</strong>基于真实的 OpenStreetMap 地理数据，涵盖参与供应链的全部 16 个国家。节点颜色与大小反映每个国家的结构性脆弱度——一个纯粹由其环节参与度推导出、不随时间变化、从不手动设定的分数；悬停时会分别显示其运营影响与已套用的灾害增量（如有）。',
    tw: '<strong>第一層 — 世界地圖。</strong>基於真實的 OpenStreetMap 地理資料，涵蓋參與供應鏈的全部 16 個國家。節點顏色與大小反映每個國家的結構性脆弱度——一個純粹由其環節參與度推導出、不隨時間變化、從不手動設定的分數；懸停時會分別顯示其營運影響與已套用的災害增量（如有）。',
    ja: '<strong>レイヤー1 — ワールドマップ。</strong>実際のOpenStreetMap地理データに基づき、チェーンに参加する16か国すべてを表示します。ノードの色とサイズは各国の構造的脆弱性——工程への参加度だけから導かれ、決して手動設定されない、時間に依存しないスコア——を示します。ホバーすると、別枠の運用インパクトと適用中のハザード差分（あれば）が表示されます。',
  },
  layer2: {
  ...
  netCard: {
    en: "<strong>Explore it as a network.</strong> A view switch offers <strong>Geographic</strong>, <strong>Topology</strong>, and <strong>Split</strong>. Instead of only bilateral country links, the chain is organised around <strong>functional supply centres</strong> — country × stage, 126 of them across 7 tiers — connected by modeled stage-mediated links (share × stage-edge prior × share). Trace multi-centre routes, temporarily remove nodes/edges (everything is reversible, with undo/redo/reset), and read topology metrics — weighted degree, reachability, betweenness, node-removal impact — each labeled a topology measure, kept separate from network influence, never a calibrated risk. Modeled connectivity, not verified shipments.",
    zh: "<strong>作为网络来探索。</strong>视图切换提供<strong>地理</strong>、<strong>拓扑</strong>与<strong>分屏</strong>三种模式。供应链不再只是国家间的双边连线，而是围绕<strong>功能供应中心</strong>组织——“国家×环节”，共 126 个、跨 7 个层级——由建模的环节中介连接相连（份额 × 环节边先验 × 份额）。你可以追踪跨中心的路线、临时移除节点/边（一切均可逆，支持撤销/重做/重置），并查看拓扑指标——加权度、可达性、介数中心性、节点移除影响——每个都标注为拓扑度量，与网络影响力分开，绝非经过校准的风险。这是建模的连通性，而非经核实的实物运输。",
    tw: "<strong>作為網路來探索。</strong>視圖切換提供<strong>地理</strong>、<strong>拓撲</strong>與<strong>分割</strong>三種模式。供應鏈不再只是國家間的雙邊連線，而是圍繞<strong>功能供應中心</strong>組織——「國家×環節」，共 126 個、跨 7 個層級——由建模的環節中介連接相連（份額 × 環節邊先驗 × 份額）。你可以追蹤跨中心的路線、臨時移除節點/邊（一切均可逆，支援復原/重做/重置），並查看拓撲指標——加權度、可達性、介數中心性、節點移除影響——每個都標註為拓撲度量，與網路影響力分開，絕非經過校準的風險。這是建模的連通性，而非經核實的實物運輸。",
    ja: "<strong>ネットワークとして探索。</strong>ビュー切替で<strong>地理</strong>・<strong>トポロジー</strong>・<strong>分割</strong>を選べます。二国間リンクだけでなく、チェーンは<strong>機能供給センター</strong>——国 × 工程、7ティアにわたる126センター——を軸に構成され、モデル化された工程媒介リンク（シェア × 工程エッジ事前値 × シェア）で結ばれます。複数センターにまたがる経路の追跡、ノード／エッジの一時除去（すべて取り消し・やり直し・リセット可能）ができ、トポロジー指標——加重次数・到達可能性・媒介中心性・ノード除去影響——を確認できます。各指標はトポロジー指標として明示し、ネットワーク影響力とは分離、較正済みリスクではありません。モデル化された連結性であり、検証された出荷ではありません。",
  },
  s1Under: {
    en: 'Underneath sits one propagation engine with three uses: recorded events, the hazard overlay, and company-disruption simulations all run through the identical code path. When an event lands, it is scaled by its declared severity, its curated per-stage exposure, and a per-incident persistence profile — acute, market, staged outage recovery, standing policy, or operationally unscored — then propagated jointly downstream along a directional input-dependence prior and upstream along a separate supplier-revenue-dependence prior, with adverse and mitigating effects carried separately — every affected stage, company, and country updates at once, and multiple simultaneous shocks combine boundedly rather than simply overwriting each other.',
    zh: '底层是同一个传导引擎的三种用法：已记录事件、灾害叠加与公司中断模拟，全部走同一条代码路径。事件发生时，会按其申报的严重度、经人工判定的各环节暴露度，以及该事件自身的持续性类别（急性、市场、分阶段复原、长期政策、或不计入运营分数）进行缩放，然后作为一个整体沿方向性的输入依赖先验值向下游传导，并沿另一个独立的供应商收入依赖先验值向上游回响，且不利与缓解两个方向分别计算——所有受影响的环节、公司与国家会同时更新，多个同时发生的冲击会以有界的方式叠加，而不是简单地相互覆盖。',
    tw: '底層是同一個傳導引擎的三種用法：已記錄事件、災害疊加與公司中斷模擬，全部走同一條程式碼路徑。事件發生時，會依其申報的嚴重度、經人工判定的各環節暴露度，以及該事件自身的持續性類別（急性、市場、分階段復原、長期政策、或不計入營運分數）進行縮放，然後作為一個整體沿方向性的輸入依賴先驗值向下游傳導，並沿另一個獨立的供應商收入依賴先驗值向上游迴響，且不利與緩解兩個方向分別計算——所有受影響的環節、公司與國家會同時更新，多個同時發生的衝擊會以有界的方式疊加，而不是簡單地相互覆蓋。',
    ja: 'その基盤にあるのは、一つの伝播エンジンの3通りの使い方です：記録されたイベント、ハザード重ね合わせ、企業破綻シミュレーションはすべて同一のコードパスを通ります。イベントが発生すると、申告された深刻度、人手で判定した工程ごとの曝露度、そして事象ごとの持続性区分（急性・市場・段階的復旧・恒常政策・運用上は非採点）によって尺度が決まり、その全体が方向性を持つ入力依存性の事前値に沿って下流へ、別の上流供給者収益依存性の事前値に沿って上流へと同時に伝播します。悪化方向と緩和方向は別々に扱われます——影響を受けるすべての工程・企業・国が同時に更新され、複数の同時発生ショックは単純に上書きされるのではなく、有界な形で合成されます。',
  },
  ...
  legendC: { en: 'Copper = network influence, modeled input-dependence, and applied hazard deltas', zh: '铜色 = 网络影响力、建模的输入依赖度与已套用的灾害增量', tw: '銅色 = 網路影響力、建模的輸入依賴度與已套用的災害增量', ja: '銅色 = ネットワーク影響度、モデル化された入力依存性、ハザード差分' },
  s2Body: {
    en: "In the flow graph, <strong>edge thickness</strong> is a modeled input-dependence prior (not a measured value flow), and each stage's <strong>dot and border size</strong> is its network influence — a Leontief-style sensitivity proxy, not a raw path count. The <strong>WHAT CHANGED</strong> strip at the top always summarizes the current state in one line, tagged structural, operational, or hazard-delta — it turns amber when a hazard overlay is applied and copper while you are reviewing a past date, and never rewrites the historical baseline.",
    zh: '在产业流图中，<strong>连线粗细</strong>是建模出的输入依赖先验值（并非实测的价值流量），每个环节的<strong>圆点与边框大小</strong>是其网络影响力——一种 Leontief 风格的敏感性代理指标，而非原始路径计数。顶部的<strong>"发生了什么变化"</strong>栏始终用一行概括当前状态，并标注为结构性、运营性或灾害增量——应用灾害叠加时变为琥珀色，回溯过去日期时变为铜色，且从不改写历史基线。',
    tw: '在產業流圖中，<strong>連線粗細</strong>是建模出的輸入依賴先驗值（並非實測的價值流量），每個環節的<strong>圓點與邊框大小</strong>是其網路影響力——一種 Leontief 風格的敏感度代理指標，而非原始路徑計數。頂部的<strong>「發生了什麼變化」</strong>欄始終用一行概括當前狀態，並標註為結構性、營運性或災害增量——套用災害疊加時變為琥珀色，回溯過去日期時變為銅色，且從不改寫歷史基線。',
    ja: 'フローグラフでは、<strong>エッジの太さ</strong>はモデル化された入力依存性の事前値であり（実測された価値の流れではありません）、各工程の<strong>ドットと枠のサイズ</strong>はそのネットワーク影響度——Leontief型の感度プロキシであり、単純な経路数ではありません。上部の<strong>「WHAT CHANGED」</strong>帯は常に現在の状態を一行で要約し、構造的・運用的・ハザード差分のいずれかのタグが付きます——ハザード重ね合わせが有効なときは琥珀色、過去の日付を確認しているときは銅色に変わりますが、過去の基準系列を書き換えることは決してありません。',
  },
  ...
  step5: { en: 'Review the past, or test a hazard', zh: '回溯过去，或测试一场灾害', tw: '回溯過去，或測試一場災害', ja: '過去を振り返る、あるいはハザードを試す' },
  step5Body: {
    en: 'The dashboard reads live. Drag the <strong>history slider</strong> — or click an event marker — and the whole model re-derives as it stood on that date: a real past state of the record, not a hypothesis. Marker height is what that event actually contributed to the index on its own day. For a hypothesis there is exactly one tool: <strong>⌖ HAZARD</strong> on the map. Drop an epicentre, set a radius and a severity, and one bounded shock runs through the identical propagation engine. It is shown in amber, labelled, and clears in one click — the preset crises and the scenario builder were removed, because a tool whose events are dated and sourced should not spend its interface on invented ones.',
    zh: '仪表盘默认呈现实时读数。拖动<strong>历史滑杆</strong>，或点击事件标记，整个模型会按该日期重新推导：这是记录中真实的过去状态，而非假设。标记的高度代表该事件当天对指数的实际贡献。若要做假设，工具只有一个：地图上的 <strong>⌖ HAZARD</strong>。落下震央、设定半径与严重度，一次有界的冲击便会通过完全相同的传导引擎运算。它以琥珀色标示，并可一键清除——预设危机情景与情景构建器已被移除，因为一个事件皆有日期与来源的工具，不该把界面花在虚构事件上。',
    tw: '儀表板預設呈現即時讀數。拖動<strong>歷史滑桿</strong>，或點擊事件標記，整個模型會按該日期重新推導：這是記錄中真實的過去狀態，而非假設。標記的高度代表該事件當天對指數的實際貢獻。若要做假設，工具只有一個：地圖上的 <strong>⌖ HAZARD</strong>。落下震央、設定半徑與嚴重度，一次有界的衝擊便會通過完全相同的傳導引擎運算。它以琥珀色標示，並可一鍵清除——預設危機情境與情境建構器已被移除，因為一個事件皆有日期與來源的工具，不該把介面花在虛構事件上。',
    ja: 'ダッシュボードは既定でライブの読み取りを表示します。<strong>履歴スライダー</strong>をドラッグする、またはイベントマーカーをクリックすると、モデル全体がその日付時点の状態として再導出されます——仮定ではなく、記録上の実際の過去の状態です。マーカーの高さは、そのイベントがその日に指数へ実際に寄与した量を表します。仮定を試す手段はただ一つ、地図上の <strong>⌖ HAZARD</strong> です。震央を置き、半径と深刻度を設定すると、境界の定まった一つのショックが完全に同一の伝播エンジンを通ります。琥珀色で明示され、ワンクリックで解除できます——プリセットの危機シナリオとシナリオビルダーは削除しました。日付と出典を備えたイベントを扱う道具が、作り物のイベントに画面を割くべきではないからです。',
  },
  step5b: { en: 'Track only what matters to you', zh: '只追踪与你相关的部分', tw: '只追蹤與你相關的部分', ja: '自分に関係するものだけを追跡' },
  step5bBody: {
    en: 'The default view is the whole chain, which is the right default and the wrong daily view. The <strong>★ WATCH</strong> tab lets you follow specific companies, products (chain stages), individual plants, or a route through the chain — each row showing its standing structural reading and any live operational effect, kept separate. The list lives in your browser and is deliberately never written into a shared link.',
    zh: '默认视图是整条供应链——这作为默认设置是对的，作为日常视图却不合适。<strong>★ WATCH</strong> 标签页让你只追踪特定公司、产品（供应链环节）、单座工厂，或一条穿过链条的路线——每一行分别显示其长期结构读数与当前运营影响，两者不混为一谈。清单仅保存在你的浏览器中，且刻意不会写入分享链接。',
    tw: '預設視圖是整條供應鏈——這作為預設設定是對的，作為日常視圖卻不合適。<strong>★ WATCH</strong> 標籤頁讓你只追蹤特定公司、產品（供應鏈環節）、單座工廠，或一條穿過鏈條的路線——每一列分別顯示其長期結構讀數與當前營運影響，兩者不混為一談。清單僅保存在你的瀏覽器中，且刻意不會寫入分享連結。',
    ja: '既定の表示はチェーン全体です。既定としては正しくても、日常的な表示としては適していません。<strong>★ WATCH</strong> タブでは、特定の企業・製品（チェーンの工程）・個々の工場、またはチェーンを貫く経路だけを追跡できます。各行には恒常的な構造スコアと現在の運用影響が別々に表示されます。リストはブラウザ内にのみ保存され、共有リンクに書き込まれることは意図的にありません。',
  },
  step6: { en: 'Generate the briefing', zh: '生成简报', tw: '產生簡報', ja: 'ブリーフィングを生成' },
  step6Body: {
    en: 'The <strong>⚡ GP Briefing</strong> button composes a full intelligence briefing from the current model state — ranked by marginal hazard delta when a hazard overlay is applied, by operational impact otherwise — what changed, most-affected nodes, company leaders, country board, watch-next, ready to copy or download.',
    zh: '<strong>⚡ GP 简报</strong>按钮会根据当前模型状态生成一份完整的情报简报——应用灾害叠加时按边际灾害增量排序，否则按运营影响排序——包括发生了什么变化、受影响最大的节点、公司领跑者、国家风险榜、后续观察点，可直接复制或下载。',
    tw: '<strong>⚡ GP 簡報</strong>按鈕會根據目前模型狀態產生一份完整的情報簡報——套用災害疊加時按邊際災害增量排序，否則按營運影響排序——包括發生了什麼變化、受影響最大的節點、公司領跑者、國家風險榜、後續觀察點，可直接複製或下載。',
    ja: '<strong>⚡ GP ブリーフィング</strong>ボタンは、現在のモデル状態から完全なインテリジェンス・ブリーフィングを作成します——ハザード重ね合わせが有効な場合は限界ハザード差分で、そうでない場合は運用インパクトでランク付けされ——何が変わったか、最も影響を受けたノード、企業リーダー、国別リスクボード、次に見るべき点が含まれ、コピーやダウンロードがすぐにできます。',
  },
