...
export const T = {
  ...
  s1Under: {
    ...
    zh: '底层是同一个传导引擎的三种用法：已记录事件、灾害叠加与公司中断模拟，全部走同一条代码路径。事件发生时，会以真实的 12 天半衰期从其申报的严重度与发生时间开始衰减，沿方向性的输入依赖先验值向下游传导，并沿另一个独立的供应商收入依赖先验值向上游回响——所有受影响的环节、公司与国家会同时更新，多个同时发生的冲击会以有界的方式叠加，而不是简单地相互覆盖。',
    tw: '底層是同一個傳導引擎的三種用法：已記錄事件、災害疊加與公司中斷模擬，全部走同一條程式碼路徑。事件發生時，會以真實的 12 天半衰期從其申報的嚴重度與發生時間開始衰減，沿方向性的輸入依賴先驗值向下游傳導，並沿另一個獨立的供應商收入依賴先驗值向上游迴響——所有受影響的環節、公司與國家會同時更新，多個同時發生的衝擊會以有界的方式疊加，而不是簡單地相互覆蓋。',
    ja: 'その基盤にあるのは、一つの伝播エンジンの3通りの使い方です：記録されたイベント、ハザード重ね合わせ、企業破綻シミュレーションはすべて同一のコードパスを通ります。イベントが発生すると、申告された深刻度と経過日数から真の12日半減期で減衰し、方向性を持つ入力依存性の事前値に沿って下流へ伝播し、別の上流供給者収益依存性の事前値に沿って上流へも反響します——影響を受けるすべての工程・企業・国が同時に更新され、複数の同時発生ショックは単純に上書きされるのではなく、有界な形で合成されます。',
  },
