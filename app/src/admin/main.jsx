...
function App() {
  ...
  const decide = async (action) => {
    ...
    try {
      ...
      setNote(`Recorded. ${result.unpublished} decision(s) awaiting publication.`);
      await load('queue');
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  };
  /* One commit per review session, not per click — work the queue, then publish. */
  const publish = async () => {
    setBusy(true); setError('');
    try {
      const result = await api('/review/publish', token, { method: 'POST' });
      setNote(result.published ? `Published: ${result.message}` : `Publish failed — decisions are still saved. ${result.error || ''}`);
      await load(tab);
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  };
