...
export const EVENT_ASSUMPTIONS = Object.freeze({
  ...
  p260729_web6934: Object.freeze({ direction: "adverse", channel: "upstream", operational: false, reason: "Sony's Kumamoto image-sensor fab halted indefinitely — the single-operator detail of the same earthquake, and the most severe individual outcome within it. Same incident as h2607_kumamoto — the M7.1 Kumamoto earthquake of Jul 28, 2026 — reported by a different source. Recorded as a non-scoring UPDATE of that primary incident rather than as an event of its own: several representations of one earthquake would otherwise each contribute and inflate the reading for a disruption that happened once. The model now deduplicates by incident before scoring, so only the primary record of an incident carries source mass; this classification records the same judgement in the data. The primary record carries the severity; this record keeps its own source, its own reporting, and its own assessment, and is displayed alongside it. See EVENT_INCIDENTS below." }),
...
/* ====================================================================
   ...
   THE PROBLEM THIS SOLVES. Distinct incidents accumulate through a
   bounded aggregation operator that is monotone: a second adverse
   contribution can only raise the total. That is the behaviour you want
   for two separate disruptions and exactly the wrong behaviour for
   several reports of one. The July 28–29, 2026 M7.1 Kumamoto earthquake
   arrived as a curated record plus six ingested news records — a USGS bulletin,
   ...
   readable but not independently accumulated. In v7 this is enforced by
   the engine itself: records are grouped by incident BEFORE any source
   vector is built (engine/eventSource.js groupIncidents), so an update
   cannot carry source mass even if its classification says otherwise.
