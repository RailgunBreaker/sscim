...
/* ================= Stage-level spread tree (hop columns) =================
   `field` is the signed operational field (positive=adverse, negative=
   mitigating); rows are ranked by modeled CONTRIBUTION (share-weighted —
   see engine/index.js companyContribution), not a share-canceling exposure
   number. */
export default function SpreadTree({ sourceStages, field, exclude, setSel, title }) {
  const { data, engine } = useVault();
  const { COMPANY_BY_ID } = data;
  const { STAGE_BY_ID, companySpread } = engine;
  const hops = companySpread(sourceStages, field, exclude);
  ...
  return (
    <div>
      <div className="mono" style={{ fontSize: 11, color: C.copper, margin: "10px 0 5px" }}>
        {title || "IMPACT SPREAD · COMPANY → COMPANY (modeled contribution = share × adverse impact × economic weight)"}
      </div>
        ...
        {hops.map((rows, h) => (
          <Fragment key={h}>
            ...
            <div style={{ minWidth: 148, flex: 1 }}>
              <div className="mono" style={{ fontSize: 11, color: C.faint, marginBottom: 4 }}>{titles[h]}</div>
              {rows.length === 0 && <div className="mono" style={{ fontSize: 11, color: C.faint }}>—</div>}
              {rows.map((r) => {
                ...
                return (
                  <div key={r.cid} className="evcard" onClick={() => setSel({ type: "company", id: r.cid })}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 6 }}>
                      ...
                      <span className="mono" style={{ fontSize: 11, color: riskColor(r.contribution * 10), fontWeight: 600 }}>{r.contribution.toFixed(3)}</span>
                    </div>
                    <div className="mono" style={{ fontSize: 11, color: C.faint }}>via {STAGE_BY_ID[r.sid].name}</div>
                  </div>
                );
              })}
            </div>
