...
export default function HazardPanel({ footprint, radiusKm, onRadiusChange, onClear, onApplyHazard }) {
  ...
  const stageRow = (s, material) => (
    <li key={s.stageId} style={{
      ...
      <span className="mono" style={{ fontSize: 12, color: material ? C.copper : C.faint }}>{pct(s.exposure)}</span>
      <span className="mono" style={{ fontSize: 12, color: C.faint, width: 62, textAlign: 'right' }}>
  ...
  return (
    <div style={{ marginTop: 10, border: `1px solid ${C.line}`, borderRadius: 6, background: C.panel2, padding: '9px 11px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <span className="mono" style={{ fontSize: 12, color: C.amber }}>⌖ HAZARD FOOTPRINT</span>
        <span className="mono" style={{ fontSize: 12, color: C.dim }}>
          {footprint.center.lat.toFixed(2)}°, {footprint.center.lng.toFixed(2)}°
        </span>
        <label className="mono" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: C.dim }}>
          RADIUS
        ...
        <button type="button" onClick={onClear}
          style={{ marginLeft: 'auto', fontSize: 12, padding: '3px 9px', borderRadius: 4, fontFamily: 'inherit', cursor: 'pointer', background: 'transparent', color: C.dim, border: `1px solid ${C.line}` }}>
          Clear
        </button>
      </div>

      {!hits.length ? (
        <div className="mono" style={{ fontSize: 12, color: C.faint, marginTop: 8, lineHeight: 1.6 }}>
        ...
        <>
          <div className="mono" style={{ fontSize: 12, color: C.dim, marginTop: 7, lineHeight: 1.6 }}>
            ...
            {hits.map(({ facility, distanceKm }) => (
              <li key={facility.id} style={{ border: `1px solid ${C.line}`, borderRadius: 4, padding: '4px 8px', background: C.panel }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                  <span style={{ flex: 1, fontSize: 12, color: C.text }}>{facility.name}</span>
                  <span className="mono" style={{ fontSize: 12, color: C.faint }}>{Math.round(distanceKm)} km</span>
                </div>
                <div style={{ fontSize: 12, color: C.dim, lineHeight: 1.5, marginTop: 1 }}>{facility.output}</div>
                <div className="mono" style={{ fontSize: 12, color: C.faint, marginTop: 2 }}>
          ...
          {/* --- stage exposure --- */}
          <div className="mono" style={{ fontSize: 12, color: C.faint, margin: '9px 0 4px' }}>
          ...
          <div className="mono" style={{ fontSize: 12, color: C.faint, marginTop: 4, lineHeight: 1.6 }}>
              ...
              <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: 2, maxHeight: 108, overflowY: 'auto' }}>
                {severedPartners.slice(0, 10).map(({ facility, weight }) => (
                  <li key={facility.id} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '2px 7px', border: `1px solid ${C.line}`, borderRadius: 4 }}>
                    <span aria-hidden style={{ fontSize: 12 }}>{flagEmoji(facility.country)}</span>
                    ...
                    <span className="mono" style={{ fontSize: 12, color: C.copper }}>{pct(Math.min(1, weight))}</span>
                  </li>
                ))}
              </ul>
              <div className="mono" style={{ fontSize: 12, color: C.faint, marginTop: 4, lineHeight: 1.6 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 9, flexWrap: 'wrap' }}>
            ...
            <span className="mono" style={{ fontSize: 12, color: C.faint, flex: '1 1 200px', lineHeight: 1.6 }}>
