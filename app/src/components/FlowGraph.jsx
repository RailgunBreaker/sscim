...
/* ================= Flow Graph + stage subsection =================
   Node color/size = STRUCTURAL VULNERABILITY (time-invariant). The +Δ
   badge = the current OPERATIONAL scenario delta — a separate, signed,
   event-driven number that is never blended into the structural color.
   Edge width = the downstream input-dependence proxy D[b][a] (a
   transparent equal-allocation prior — see engine/graph.js — NOT a
    ...
    <div style={{ padding: 10 }}>
      <div style={{ overflowX: "auto" }}>
        <svg viewBox="0 0 1220 505" style={{ width: "100%", minWidth: 860, display: "block", background: C.panel2, borderRadius: 6, border: `1px solid ${C.line}` }}>
          ...
          {STAGES.map((st) => {
            const structural = STRUCTURAL_VULNERABILITY[st.id];
            const col = riskColor(structural);
            ...
            return (
              <g key={st.id} className="node" opacity={dimAll && !active ? 0.3 : 1} onClick={() => setSel({ type: "stage", id: st.id })}
                ...
                <text x={st.x - 14} y={st.y + 12} textAnchor="middle" fill={col} fontSize="9.5" className="mono" fontWeight="600">
                  {structural.toFixed(1)}{scenarioActive && Math.abs(delta) > 0.01 ? ` ${delta >= 0 ? "+" : ""}${delta.toFixed(2)}Δ` : ""}
                </text>
      ...
      <Legend items={[["Edge width = modeled input-dependence weight (prior)", C.copperDim], ["Dot & border size = network influence", C.copper]]}
        note="Color = structural vulnerability · +Δ = current operational scenario delta · tap a stage → subsection · tap a company → detail" />
    </div>
  );
}
