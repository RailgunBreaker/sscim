  ...
  return (
    <div style={{ marginTop: 10 }}>
          ...
          return (
            <div key={f.id} style={{ border: `1px solid ${C.line}`, borderRadius: 4, padding: '4px 8px', background: C.panel }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ flex: 1, fontSize: 11.5, color: C.text, lineHeight: 1.35 }}>{f.name}</span>
                <span className="mono" style={{ fontSize: 10, color: siteWeight(f) ? C.copper : C.faint }}>
                  {siteWeight(f) ? `${Math.round(share * 100)}%` : 'not yet running'}
                </span>
              </div>
              <div style={{ fontSize: 9.5, color: C.dim, lineHeight: 1.5, marginTop: 1 }}>{f.output}</div>
      ...
      {sites.length > MAX_SHOWN && (
        <div className="mono" style={{ fontSize: 9, color: C.faint, marginTop: 4 }}>
