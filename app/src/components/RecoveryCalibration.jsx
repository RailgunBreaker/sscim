...
export default function RecoveryCalibration({ report }) {
  ...
  return <section aria-label="Recovery duration calibration">
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr><th style={cell}>Predictor</th><th style={cell}>Mean absolute error (days)</th></tr></thead>
      <tbody>{[['Fitted recovery duration','candidate'],['Archived recovery assumption','registry'],['Earlier empirical median','empiricalMedian'],['Latest earlier incident','lastIncident']].map(([label,key]) => <tr key={key}><td style={cell}>{label}</td><td style={cell}>{number(test.metrics[key]?.maeDays)}</td></tr>)}</tbody>
    </table>
    ...
    <p>The dashboard does not use the archived global assumptions to score companies. These endpoints do not validate the recovery trajectory, lost output, supplier transmission or chain-wide financial losses. Evaluation date: {report.asOf}.</p>
  </section>;
}
