            ...
            <div style={{ minWidth: 148, flex: 1 }}>
              ...
              {col.rows.map((r) => (
                <div key={r.cid} className="evcard" onClick={() => setSel({ type: "company", id: r.cid })}
                  ...
                  {h > 0 && <div className="mono" style={{ fontSize: 11, color: C.faint }}>{h === 1 ? "buys" : "path weight"} {(r.rel * 100).toFixed(0)}% <span style={{ color: C.faint }}>(supplier-revenue share)</span></div>}
                </div>
              ))}
            </div>
          </Fragment>
