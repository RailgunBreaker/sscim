...
const STYLE = `
  .nt{border-bottom:1px solid var(--line);background:var(--panel2);display:flex;align-items:center;gap:0;overflow:hidden;position:relative;font-size:11px}
  ...
  .nt-track{flex:1;overflow:hidden;position:relative;-webkit-mask-image:linear-gradient(90deg,transparent,#000 28px,#000 calc(100% - 46px),transparent);mask-image:linear-gradient(90deg,transparent,#000 28px,#000 calc(100% - 46px),transparent)}
  .nt-rail{display:inline-flex;align-items:center;white-space:nowrap;will-change:transform;animation:nt-roll linear infinite}
  .nt:hover .nt-rail,.nt:focus-within .nt-rail{animation-play-state:paused}
  .nt-item{display:inline-flex;align-items:center;gap:7px;padding:8px 26px 8px 0;color:var(--dim)}
  ...
  @media(prefers-reduced-motion:reduce){
    .nt-rail{animation:none}
    .nt-track{overflow-x:auto}
  }
`;
...
export default function NewsTicker({ note = 'Not investment advice' }) {
  ...
  const rail = (
    <span className="nt-rail" style={{ animationDuration: `${duration}s` }}>
      {[0, 1].map((copy) => (
        <span key={copy} aria-hidden={copy === 1} style={{ display: 'inline-flex' }}>
          {items.map((e) => (
